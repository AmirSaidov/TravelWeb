...
const TourDetail = () => {
  ...
  const shareTour = async () => {
    const shareData = { title: tour.title, text: tour.shortDescription, url: window.location.href };
    try {
      if (navigator.share) {
        await navigator.share(shareData);
      } else {
        await navigator.clipboard.writeText(window.location.href);
        toast({ title: "Link copied", description: "Tour URL was copied to clipboard." });
      }
    } catch {
  ...
  return (
    <motion.div className="container-page py-10" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex gap-3 text-sm">
          <button onClick={shareTour} className="inline-flex items-center gap-1.5 rounded-full px-3 py-2 transition hover:bg-muted active:scale-[0.98]"><Share2 className="h-4 w-4" />{t("tour.share")}</button>
          <button
            onClick={() => {
              toggleSave(tour.id);
              setHeartFxKey((k) => k + 1);
            }}
            className="relative inline-flex items-center gap-1.5 rounded-full px-3 py-2 transition hover:bg-muted active:scale-[0.98]"
          >
      ...
      <div className="mt-6 grid gap-3 overflow-hidden rounded-3xl md:grid-cols-3">
        ...
        <div className="relative aspect-[4/3] overflow-hidden bg-muted">
          ...
          <button onClick={() => toast({ title: "Gallery", description: "Full gallery will be available in the next update." })} className="absolute bottom-3 right-3 rounded-xl bg-white/95 px-3 py-1.5 text-xs font-medium text-foreground shadow-card hover:bg-white">📷 {t("tour.showAll")}</button>
        </div>
      </div>
      ...
      <div className="mt-10 grid gap-10 lg:grid-cols-[1fr_380px]">
        <div>
          ...
          <div className="border-b border-border py-6">
            ...
            {!showMore && <button onClick={() => setShowMore(true)} className="mt-3 inline-flex items-center gap-1 text-sm font-semibold underline-offset-4 hover:underline">{t("tour.showMore")} ›</button>}
          </div>

          <div className="py-6">
            <h2 className="font-display text-xl font-semibold">{t("tour.included")}</h2>
            <ul className="mt-4 grid gap-3 sm:grid-cols-2">
              {tour.included.map((x) => <li key={x} className="flex items-center gap-2 text-sm"><Check className="h-4 w-4 text-brand" />{x}</li>)}
            </ul>
          </div>
                ...
                {tReviews.map((r) => (
                  <div key={r.id} className="rounded-2xl border border-border p-5">
                    <div className="flex items-center gap-3">
                      ...
                      <div><div className="text-sm font-semibold">{r.author}</div><div className="text-xs text-muted-foreground">{r.date}</div></div>
                    </div>
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <div className="rounded-3xl bg-card p-6 shadow-elevated ring-1 ring-border">
            <div className="flex items-end justify-between">
              <div><span className="font-display text-3xl font-semibold">${tour.price}</span><span className="text-sm text-muted-foreground"> / person</span></div>
              <div className="text-right text-xs text-muted-foreground"><RatingStars value={tour.rating} /><div>{tour.reviewCount} {t("tour.reviews")}</div></div>
            </div>

            <div className="mt-4 overflow-hidden rounded-2xl border border-border">
              <div className="grid grid-cols-2">
                <label className="border-r border-border p-3"><span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{t("tour.startDate")}</span><input type="date" value={start} onChange={(e) => setStart(e.target.value)} className="mt-1 w-full bg-transparent text-sm font-medium outline-none" /></label>
                <label className="p-3"><span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{t("tour.endDate")}</span><input type="date" value={end} onChange={(e) => setEnd(e.target.value)} className="mt-1 w-full bg-transparent text-sm font-medium outline-none" /></label>
              </div>
              <label className="block border-t border-border p-3"><span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{t("tour.guestsLabel")}</span><input type="number" min={1} max={tour.maxGuests} value={guests} onChange={(e) => setGuests(Math.max(1, Math.min(tour.maxGuests, +e.target.value || 1)))} className="mt-1 w-full bg-transparent text-sm font-medium outline-none" /></label>
            </div>
            ...
            <div className="mt-3 flex items-center justify-between border-t border-border pt-3"><span className="font-semibold">{t("tour.total")}</span><span className="font-display text-lg font-semibold">${total.toLocaleString()}</span></div>
            <div className="mt-4 rounded-xl bg-brand-soft px-3 py-2 text-center text-xs font-medium text-accent-foreground">{t("tour.freeCancellation", { date: "Aug 7th" })}</div>
          </div>
        </aside>
      </div>
    </motion.div>
