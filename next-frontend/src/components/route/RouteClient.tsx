...
export function RouteClient() {
  ...
  const { data: toCoord = null } = useQuery({
  ...
  const { data: route = null, isLoading: routeLoading, isError: routeError } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { geometry: r.geometry, distance: Number(r.distance) || 0, duration: Number(r.duration) || 0 };
    },
  });
    ...
    return (
      <div className="container-page py-10">
        <div className="rounded-3xl border border-dashed border-[hsl(var(--border))] p-8 text-center text-sm text-[hsl(var(--muted-foreground))]">
          Destination missing. Go back to{" "}
          <Link className="text-[hsl(var(--brand))] hover:underline" href="/dashboard">
            Dashboard
          </Link>
          .
        </div>
      ...
      <div className="container-page grid gap-6 py-6 lg:grid-cols-[420px_1fr]">
        <aside className="rounded-3xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-6 shadow-card">
          <div className="flex items-center justify-between">
            ...
            <Link
              href="/dashboard"
              className="inline-flex h-9 items-center justify-center rounded-full border border-[hsl(var(--input))] bg-[hsl(var(--background))] px-4 text-xs font-semibold hover:bg-[hsl(var(--accent))]"
            >
              Back
            </Link>
          </div>
          <div className="mt-2 text-sm text-[hsl(var(--muted-foreground))]">{toText}</div>
          ...
          <div className="mt-6 space-y-2">
            ...
            <div className="relative">
              <input
                ...
                className="h-11 w-full rounded-xl border border-[hsl(var(--input))] bg-[hsl(var(--background))] px-3.5 text-sm outline-none placeholder:text-[hsl(var(--muted-foreground))]"
              />
              {suggestOpen && suggestions.length > 0 && (
                <div className="absolute left-0 right-0 top-[calc(100%+0.5rem)] z-20 overflow-hidden rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--background))] shadow-elevated">
                  {suggestions.map((s) => (
                    <button
                      key={`${s.lng}:${s.lat}:${s.title}`}
                      type="button"
                      onClick={() => {
                      ...
                      className="w-full px-4 py-3 text-left text-sm hover:bg-[hsl(var(--accent))]"
                    >
                      <div className="font-medium">{s.title}</div>
                      {s.subtitle && <div className="mt-0.5 text-xs text-[hsl(var(--muted-foreground))]">{s.subtitle}</div>}
                    </button>
                  ))}
                </div>
              )}
            ...
            <button
              type="button"
              onClick={geocodeFromText}
              className="inline-flex h-10 w-full items-center justify-center rounded-xl bg-[hsl(var(--primary))] px-4 text-sm font-semibold text-[hsl(var(--primary-foreground))] hover:opacity-95"
            >
              Build route
            </button>
            ...
            <div className="mt-4 rounded-2xl bg-[hsl(var(--muted))]/40 p-4 text-sm">
              {routeLoading ? (
                <div className="text-[hsl(var(--muted-foreground))]">Loading route…</div>
              ) : routeError ? (
                <div className="text-red-600">Failed to build route.</div>
              ) : route ? (
                <div className="space-y-1">
                  ...
                  <div>
                    Duration: <span className="font-semibold">{durationMin} min</span>
                  </div>
                </div>
              ) : (
                <div className="text-[hsl(var(--muted-foreground))]">Pick a start point to see the route.</div>
              )}
            </div>
          </div>
        </aside>
