...
function buildWorldMaskFeature(outerRings: LinearRing[]) {
  ...
  // GeoJSON expects: [ outer, hole1, hole2, ... ].
  // We use one "world" outer ring and one hole per Kyrgyzstan outer ring (for multiparts).
...
export async function applyKyrgyzstanOnlyMap(
  ...
  // Bounds: union of all outer rings.
  ...
  // Clean up from previous style loads/toggles.
  ...
  const mask = {
    ...
    features: [buildWorldMaskFeature(outerRings)],
  };
