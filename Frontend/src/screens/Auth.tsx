...
export const AuthPage = ({ mode }: Props) => {
  ...
  if (mode === "forgot") {
    return (
      <AuthShell
        ...
        footer={<Link href="/login" className="text-primary hover:underline">{t("auth.signin")}</Link>}
      >
  ...
  return (
    <AuthShell
      ...
      footer={
        mode === "login"
          ? (<>{t("auth.noAccount")} <Link href="/register" className="font-semibold text-brand hover:underline">{t("auth.signup")}</Link></>)
          : (<>{t("auth.haveAccount")} <Link href="/login" className="font-semibold text-brand hover:underline">{t("auth.signin")}</Link></>)
      }
    >
      <AuthForm mode={mode} />
    </AuthShell>
